...
savepath;
a=strfind(pathstr,'/');
result_dir=strcat(pathstr(1:a(end)),'Results_glutamicum/quercertin');
mkdir(result_dir);
...
cobra_model=Get_heterologous(model,'quercetin');
...
%Export model
writeCbModel(cobra_model,'sbml',strcat(result_dir,'/','quercetin_model'));
...
[num,optgene_results,raw] = xlsread(strcat(result_dir,'/optgene_quercetin.xlsx'));
...
%For windows:
%xlswrite('quercetin_results',all_rxns,'All KOs') 
%xlswrite('quercetin_results.xls',table_subsystems,'Subsystems enrichment');
%xlswrite('quercetin_results.xls',table_all_comb,'All KO combinations');
%xlswrite('quercetin_results.xls',table_best_comb,'Best KO combinations');
...
xlwrite('quercetin_results.xls',all_rxns,'All KO reactions'); %put formulas
xlwrite('quercetin_results.xls',table_subsystems,'Subsystems enrichment');
xlwrite('quercetin_results.xls',table_results,'All KO combinations');
xlwrite('quercetin_results.xls',table_best_results,'Best KO combinations');
