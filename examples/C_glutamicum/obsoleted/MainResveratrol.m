...
savepath;
a=strfind(pathstr,'/');
result_dir=strcat(pathstr(1:a(end)),'Results_glutamicum/resveratrol');
mkdir(result_dir);
...
cobra_model=Get_heterologous(model,'resveratrol');
...
[optknock_results]=runOptknock(cobra_model,selectedRxnIDs,BM_rxn,target_rxn);
...
%Export model
writeCbModel(cobra_model,'sbml',strcat(result_dir,'/','resveratrol_model'));
...
[num,optgene_results,raw] = xlsread(strcat(result_dir,'/optgene_resv.xlsx'));
...
%For windows:
%xlswrite('resveratrol_results',all_rxns,'All KOs') 
%xlswrite('resveratrol_results.xls',table_subsystems,'Subsystems enrichment');
%xlswrite('resveratrol_results.xls',table_all_comb,'All KO combinations');
%xlswrite('resveratrol_results.xls',table_best_comb,'Best KO combinations');
...
xlwrite('resveratrol_results.xls',all_rxns,'All KO reactions'); %put formulas
xlwrite('resveratrol_results.xls',table_subsystems,'Subsystems enrichment');
xlwrite('resveratrol_results.xls',table_results,'All KO combinations');
xlwrite('resveratrol_results.xls',table_best_results,'Best KO combinations');
