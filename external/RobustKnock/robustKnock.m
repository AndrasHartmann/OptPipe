function results=robustKnock(consModel, chemicalInd, objectiveInd, knockableRxns, knockoutNum, maxW, optKnockFlag, robustKnockFlag )
...
K=knockoutNum;
[model,yInd, notYInd, m,n, coupled, coupledYInd, coupledNotYInd]=createRobustOneWayFluxesModel(consModel, chemicalInd, coupledFlag, knockableRxns);
ySize=size(yInd,1);
...
if (optKnockFlag ==1)
    ...
    Result_optKnock = runMILP(Cjoined, Ajoined, Bjoined, ...
                              lbJoined, ubJoined, IntVars_optKnock);

    YsOptKnockNotRound=Result_optKnock.full(IntVars_optKnock);
    YsOptKnock=round(YsOptKnockNotRound);
    knockedYind=find(1.-YsOptKnock);
    knockedVoptKnock=yInd(knockedYind);

    results.Result_optKnock.exitFlag=Result_optKnock.stat;
    ...
    results.Result_optKnock.objective=Result_optKnock.full(model.organismObjectiveInd);
    results.Result_optKnock.chemicel=Result_optKnock.full(model.chemicalInd);
    results.Result_optKnock.x_k=Result_optKnock.full;
    ...
    results.Result_optKnock.knockedVvalues=Result_optKnock.full(yInd(results.Result_optKnock.knockedYind));
    results.Result_optKnock.y=YsOptKnockNotRound;
    results.Result_optKnock.knockedVNames= model.rxnNames(knockedVoptKnock);
    results.Result_optKnock.knockedVNames

    %if(Result_optKnock.stat==1)        %a sucessful run
        results.Result_optKnock.knockCheck=knockCheck(model, knockedVoptKnock, coupledFlag, coupled);
...
Result_OptKnock2 = runMILP(C3, A3, B3, lb3, ub3, intVars);
...
results.Result_OptKnock2.chemical=-Result_OptKnock2.obj;
results.Result_OptKnock2.ySize=ySize;
results.Result_OptKnock2.ySum=sum(Result_OptKnock2.full(intVars));
...
results.Result_OptKnock2.knockedVNames= model.rxnNames(knockedVindRobustKnock);
results.Result_OptKnock2.knockedVNames;


%if(Result_OptKnock2.stat==1)        %a sucessful run
    results.Result_OptKnock2.knockCheck=knockCheck(model, knockedVindRobustKnock, coupledFlag, coupled);
