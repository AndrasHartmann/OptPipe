function [A_w,Ay_w ,B_w,C_w, lb_w, ub_w, wSize, wZs]=seperateTransposeJoin(A, Ay, B,C,ySize, yMax, aSizeCol, highNum, findMaxFlag, zSize)
...
for k=1:wSize
    ...
    if (maxVal~=0  || minVal~=0)
        if (findMaxFlag==1)
            ...
            Result = runLP(-c_k, A', [], C, zeros(wSize,1), []);

            %Prob=lpAssign(-c_k, A', [], C, zeros(wSize,1), []);
                             %c, A, b_L, b_U, x_L, x_U, x_0,
            %Result=tomRun('cplex', Prob, 0);
         
            if (Result.stat==1 && Result.obj > 1e-3) %found a solution
                wMax(k)=Result.obj;
                maxW= max(maxW,res);
            end
        end
