...
range = 0:5:100;
[OTDB_min, OTDB_max] = multiFluxVariability(cobra_model,selectedRxnIDs, biomass_reaction_id, target_reaction_id, range);
...
for k = 1:length(selectedRxnIDs)
    ...
    if any(abs(target_zero) < 1e-6)
        ...
        if computeTotal > 0 && ~isinf(computeTotal)
        	reaction_index = selectedRxnIDs(k);
...
if ~isempty(ranked_reactions)
    ...
    fva_reaction_indices = arrayfun(@(x) find(selectedRxnIDs == x, 1), ranked_reactions(:,1));
